...
def quick_sort_python(sort_list):
    if len(sort_list) <= 1:
        return sort_list
    flag = sort_list[0]
    return quick_sort_python([i for i in sort_list[1:] if i < flag]) + [flag] + quick_sort_python([i for i in sort_list[1:] if i >= flag])


if __name__ == '__main__':
    list = [4, 6, 8, 1, 3]
    quick_sort_all(list, 0, len(list) - 1)
    print(quick_sort_python(list))
