...
def circle_matrix(n):
    ...
    for k in range(0, n//2):
        for i in range(k, n-k):
            base += 1
            circle_matrix[k][i] = base
        for j in range(k+1, n-1-k):
            base += 1
            circle_matrix[j][n-1-k] = base
        for i in range(n-1-k, k, -1):
            base += 1
            circle_matrix[n-1-k][i] = base
        for j in range(n-1-k, k, -1):
            base += 1
            circle_matrix[j][k] = base
    if(n%2 == 1):
        base += 1
        circle_matrix[(n-1)//2][(n-1)//2] = base
    for i in range(0, n):
        for j in range(0, n):
            print('{}'.format(circle_matrix[i][j]), end='   ')
        print('\n')
